defmodule Day01 do
  ...
  def solve1 do
    Input.read("day01")
      |> String.split(", ")
      |> Enum.reduce(%Telemetry{}, &SantaMotion.process/2)
      |> TaxicabGeometry.distance
  end
  ...
  def solve2 do
    Input.read("day01")
      |> String.split(", ")
      |> Enum.reduce_while(%Telemetry{}, &move_and_test_intersection/2)
      |> TaxicabGeometry.distance
  end

  defp move_and_test_intersection(instruction, telemetry) do
    telemetry = SantaMotion.process(instruction, telemetry)

    case telemetry.intersection do
      nil -> {:cont, telemetry}
      loc -> {:halt, loc}
    end
  end
